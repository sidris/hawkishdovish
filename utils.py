...
HAWK_DOVE_DICT_STRUCT = {
   ...
   # (Tam sözlük yukarıda olduğu için burada kısalttım, asıl kodda tam hali olmalı)
} 
# Not: Sözlük zaten yukarıda "DICT" olarak tanımlı, "analyze_hawk_dove_structural" fonksiyonu da "DICT" kullanabilir 
# veya "utils.py" dosyanızın başındaki "DICT" yapısını kullanabilirsiniz. 
# "utils.py" içindeki "analyze_hawk_dove_structural" fonksiyonunun eksiksiz hali:

def normalize_text_struct(t): return normalize_text(t)
def split_sentences_struct(t): return split_sentences_nlp(t)
def analyze_hawk_dove_structural(text, window_words=7, dedupe_within_term_window=True, nearest_only=True):
    return analyze_hawk_dove(text, DICT, window_words, dedupe_within_term_window, nearest_only)
...
def analyze_with_roberta(text):
    ...
    try:
        res = clf(text[:2000])[0]
        # Moritz-Pfeifer labels: hawkish, dovish, neutral
...
def analyze_sentences_with_roberta(text):
    ...
    sents = [s for s in split_sentences_nlp(text) if len(s.split()) > 3]
    ...
    try:
        ...
        for s, p in zip(sents, preds):
            # p, [{'label':..., 'score':...}, ...] listesi olabilir
            best = max(p, key=lambda x: x['score']) if isinstance(p, list) else p
            res_list.append({"Cümle": s, "Etiket": tr_map.get(best['label'].lower(), best['label']), "Güven Skoru": best['score'], "Ham": best['label'].lower()})
...
# =============================================================================
# 10. TARİHSEL RoBERTa HESAPLAMA (DASHBOARD İÇİN) - YENİ
# =============================================================================
@st.cache_data
def calculate_roberta_series(df):
    """Tüm geçmiş veriler için RoBERTa skoru hesaplar.
    Skorlama: Hawkish = +1, Dovish = -1, Neutral = 0.
    Güven skoru ile ağırlıklandırılır ve 100 ile çarpılır.
    Sonuç: -100 (Aşırı Güvercin) ile +100 (Aşırı Şahin) arası."""
    if df.empty: return pd.DataFrame()
    
    # Pipeline'ı yükle (Cache sayesinde hızlı gelir)
    classifier = load_roberta_pipeline()
    if not classifier: return pd.DataFrame()

    results = []
    # İlerlemeyi göstermek isterseniz st.progress kullanabilirsiniz ama cache_data içinde riskli olabilir.
    # Doğrudan döngü:
    for _, row in df.iterrows():
        ...
        try:
            # Model çağrısı
            res = classifier(text[:2000])[0] # [{'label':'hawkish', 'score':0.9}, ...]
            ...
            for item in res:
                ...
                scr = item['score']
                if 'hawkish' in lbl:
                    final_val += scr # Hawkish pozitif etki
                elif 'dovish' in lbl:
                    final_val -= scr # Dovish negatif etki
                # Neutral 0 etkisi (değiştirmez)
            
            # Model çıktısı genelde softmax olduğu için toplam 1'dir.
            # Net skor -1 ile 1 arasında olacaktır.
            # Dashboard için ölçekle (Örn: 100 ile çarp)
            scaled_score = final_val * 100.0
            
            results.append({
                "period_date": row.get("period_date"),
                "roberta_index": scaled_score
            })
        except:
            continue
