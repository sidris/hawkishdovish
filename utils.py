...
def detect_policy_action(text: str) -> dict:
    """
    TR+EN metinden politika aksiyonu + büyüklük (bp) çıkarır.

    Return örnek:
      {"action":"HIKE","bp":500,"weight_0_1":1.0,"method":"from_to"}
      {"action":"HOLD","bp":0,"weight_0_1":0.0,"method":"hold_keyword"}
      {"action":"UNKNOWN","bp":None,"weight_0_1":None,"method":"none"}
    """
    if not text:
        return {"action": "UNKNOWN", "bp": None, "weight_0_1": None, "method": "none"}
    ...
    # 1) from X to Y  /  45'ten 50'ye  /  %45'ten %50'ye
    m_en = re.search(r"\bfrom\s+(\d+(?:[\.,]\d+)?)\s*%?\s+to\s+(\d+(?:[\.,]\d+)?)\s*%?\b", t)
    m_tr = re.search(r"\b(\d+(?:[\.,]\d+)?)\s*%?\s*(?:'ten|ten)\s+(\d+(?:[\.,]\d+)?)\s*%?\s*(?:'ye|ye)\b", t)

    if m_en or m_tr:
        m = m_en if m_en else m_tr
        a = _to_float(m.group(1))
        b = _to_float(m.group(2))
        if a is not None and b is not None:
            bp = int(round((b - a) * 100))  # 1 puan = 100 bp
            if bp > 0:
                action = "HIKE"
            elif bp < 0:
                action = "CUT"
                ...
                action = "HOLD"
            w = min(abs(bp) / 500.0, 1.0)  # 500bp = 1.0
            return {"action": action, "bp": bp, "weight_0_1": float(w), "method": "from_to"}

    # 2) "500 bp" / "500 baz puan"
    m_bp = re.search(r"\b(\d{1,4})\s*(?:bp|bps|baz\s*puan|bazpuan)\b", t)
    if m_bp:
        bp = int(m_bp.group(1))

        if re.search(r"\b(indir|indirim|azalt|düşür|düşüş|reduce|cut|lower|decrease)\b", t):
            action = "CUT"
            bp = -abs(bp)
        elif re.search(r"\b(artır|artış|yükselt|yükseliş|increase|raise|hike|tighten)\b", t):
            action = "HIKE"
            bp = abs(bp)
        elif re.search(r"\b(sabit|değişmedi|değişiklik|koru|unchanged|maintain|keep)\b", t):
            action = "HOLD"
            bp = 0
        else:
            action = "UNKNOWN"
        ...
        w = min(abs(bp) / 500.0, 1.0) if bp is not None else None
        return {"action": action, "bp": bp, "weight_0_1": w, "method": "bp_token"}

    # 3) "5 puan" / "5 percentage points"
    m_pp = re.search(r"\b(\d+(?:[\.,]\d+)?)\s*(?:puan|yüzdelik\s*puan|percentage\s*points?)\b", t)
    if m_pp:
        pp = _to_float(m_pp.group(1))
        if pp is not None:
            bp = int(round(pp * 100))
            if re.search(r"\b(indir|indirim|azalt|düşür|reduce|cut|lower|decrease)\b", t):
                action = "CUT"
                bp = -abs(bp)
            elif re.search(r"\b(artır|artış|yükselt|increase|raise|hike|tighten)\b", t):
                action = "HIKE"
                bp = abs(bp)
            else:
                action = "UNKNOWN"
            ...
            w = min(abs(bp) / 500.0, 1.0) if bp is not None else None
            return {"action": action, "bp": bp, "weight_0_1": w, "method": "points_token"}

    # 4) HOLD keyword TR+EN
    if re.search(r"\b(sabit|değişmedi|değişiklik\s*yapılmadı|korunmasına|koru|unchanged|maintain|keep)\b", t) and \
       re.search(r"\b(faiz|politika\s*faizi|bir\s*hafta\s*repo|policy\s*rate|interest\s*rate)\b", t):
        return {"action": "HOLD", "bp": 0, "weight_0_1": 0.0, "method": "hold_keyword"}

    # 5) Yön var ama büyüklük yok
    if re.search(r"\b(artır|artış|yükselt|increase|raise|hike|tighten)\b", t) and \
       re.search(r"\b(faiz|politika\s*faizi|bir\s*hafta\s*repo|policy\s*rate|interest\s*rate)\b", t):
        return {"action": "HIKE", "bp": None, "weight_0_1": None, "method": "hike_keyword"}

    if re.search(r"\b(indir|indirim|azalt|düşür|reduce|cut|lower|decrease)\b", t) and \
       re.search(r"\b(faiz|politika\s*faizi|bir\s*hafta\s*repo|policy\s*rate|interest\s*rate)\b", t):
        return {"action": "CUT", "bp": None, "weight_0_1": None, "method": "cut_keyword"}
...
def _safe_split_sentences(text: str) -> list[str]:
    """
    Prod'da NLP splitter bozulursa diye sağlam fallback.
    TR/EN karışık metinlerde de çalışır.
    """
    ...
    # Nokta/ünlem/soru/iki nokta/; sonrası split
    parts = re.split(r"(?<=[\.\!\?\;:])\s+", t)
    parts = [p.strip() for p in parts if p and p.strip()]

    # Çok kısa / “Madde 1” gibi şeyleri ele (çok agresif olmasın)
    out = []
    for p in parts:
        if len(p) < 25:
            continue
...
def analyze_sentences_with_roberta(text: str) -> pd.DataFrame:
    """
    Önce mevcut splitter'ı dener (varsa), yoksa fallback.
    Sonra mrince pipeline ile sentence bazında skorları çıkarır.
    """
    if not text:
        return pd.DataFrame()
    ...
    # 1) sentence split: önce eski fonksiyon varsa dene
    sentences = []
    try:
        if "split_sentences_nlp" in globals():
    ...
    # 2) fallback
    if not sentences:
        sentences = _safe_split_sentences(str(text))
    ...
    # 4) model inference (batch)
    try:
        preds = clf(sentences[:160])  # güvenlik limiti
        rows = []

        for sent, pred in zip(sentences[:160], preds):
